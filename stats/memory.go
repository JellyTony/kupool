...
type MemoryStore struct {
	...
	jobHist     map[int]serverpkg.JobRecord
	latestJobID int
	latestNonce string
...
func NewMemoryStore() *MemoryStore {
	return &MemoryStore{data: make(map[string]map[time.Time]int), jobHist: make(map[int]serverpkg.JobRecord), userStates: make(map[string]struct {
		latestJobID int
		latestNonce string
		lastSubmit  time.Time
	}), used: make(map[string]map[int]map[string]struct{})}
}
...
func (s *MemoryStore) SaveJob(jobID int, nonce string, createdAt time.Time) error {
	...
	s.latestJobID = jobID
	s.latestNonce = nonce
	s.jobHist[jobID] = serverpkg.JobRecord{Nonce: nonce, CreatedAt: createdAt}
	return nil
}
...
func (s *MemoryStore) LoadLatestJob() (int, string, time.Time, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	rec := s.jobHist[s.latestJobID]
	return s.latestJobID, rec.Nonce, rec.CreatedAt, nil
}
...
func (s *MemoryStore) LoadJobHistory(since time.Duration) (map[int]serverpkg.JobRecord, error) {
	...
	for id, rec := range s.jobHist {
		if since == 0 || rec.CreatedAt.After(cutoff) {
			out[id] = rec
		}
	}
	return out, nil
...
func (s *MemoryStore) SaveUserState(username string, latestJobID int, latestServerNonce string, lastSubmitAt time.Time) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.userStates[username] = struct {
		latestJobID int
		latestNonce string
		lastSubmit  time.Time
	}{latestJobID, latestServerNonce, lastSubmitAt}
	return nil
}
...
func (s *MemoryStore) SaveUsedNonce(username string, jobID int, clientNonce string) error {
	...
	if !ok {
		u = make(map[int]map[string]struct{})
		s.used[username] = u
	}
	m, ok := u[jobID]
	if !ok {

...
// NewClient NewClient
func NewClient(id, name string, opts ClientOptions) kupool.Client {
	if opts.WriteWait == 0 {
		opts.WriteWait = kupool.DefaultWriteWait
	}
	if opts.ReadWait == 0 {
...
func (c *Client) Connect(addr string) error {
	...
	rawconn, err := c.Dialer.DialAndHandshake(kupool.DialerContext{
		Id:      c.id,
		Name:    c.name,
		Address: addr,
		Timeout: kupool.DefaultLoginWait,
	})
	if err != nil {
...
// SetDialer 设置握手逻辑
func (c *Client) SetDialer(dialer kupool.Dialer) {
	c.Dialer = dialer
}
...
func (c *Client) Send(payload []byte) error {
	...
	return c.conn.WriteFrame(kupool.OpBinary, payload)
}
...
func (c *Client) Close() {
	c.once.Do(func() {
		...
		_ = WriteFrame(c.conn, kupool.OpClose, nil)
...
func (c *Client) Read() (kupool.Frame, error) {
	if c.conn == nil {
	...
	if frame.GetOpCode() == kupool.OpClose {
...
func (c *Client) ping() error {
	...
	return c.conn.WriteFrame(kupool.OpPing, nil)
}

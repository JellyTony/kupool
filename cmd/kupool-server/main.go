...
func main() {
    ...
    flag.Parse()
    if v := os.Getenv("KUP_ADDR"); v != "" { *addr = v }
    if v := os.Getenv("KUP_INTERVAL"); v != "" { if d, err := time.ParseDuration(v); err == nil { *interval = d } }
    if v := os.Getenv("KUP_STORE"); v != "" { *storeKind = v }
    if v := os.Getenv("KUP_PG_DSN"); v != "" { *pgDsn = v }
    if v := os.Getenv("KUP_MQ"); v != "" { *mqKind = v }
    if v := os.Getenv("KUP_MQ_URL"); v != "" { *mqUrl = v }
    if v := os.Getenv("KUP_MQ_QUEUE"); v != "" { *mqQueue = v }
    ...
    if *storeKind == "pg" {
        ...
        if err != nil { logger.WithError(err).Fatal("pg init failed") }
        store = pg
    } else {
    ...
    if *mqKind == "rabbit" {
        ...
        if err != nil { logger.WithError(err).Fatal("rabbitmq init failed") }
        queue = r
    } else {
